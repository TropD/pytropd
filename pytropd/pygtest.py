...
#Define a time axis
time_axis = pyg.ModelTime365(values=np.arange(365*38), units = 'days', startdate=dict(year=1979, month=1, day=1))  
#data is monthly mean
time_axis = time_axis.monthlymean().time


v_data = pyg.open('./ValidationData/va.nc')
...
if not (np.max(psi_metrics(metrics=0)[:] - psi_metric_validation_data.PSI_SH[:])) < 1e-10 and \
    (np.max(psi_metrics(metrics=1)[:] - psi_metric_validation_data.PSI_NH[:])) < 1e-10:
...
T_data = pyg.open('./ValidationData/ta.nc')
...
if not (np.max(tpb_metrics(metrics=0)[:] - tpb_metric_validation_data.TPB_SH[:])) < 1e-10 and \
        (np.max(tpb_metrics(metrics=1)[:] - tpb_metric_validation_data.TPB_NH[:])) < 1e-10:
...
psl_data = pyg.open('./ValidationData/psl.nc')                                                                
...
psl_JJA_metric_validation_data = pyg.open('./ValidationMetrics/PSL_JJA.nc')    
psl_SON_metric_validation_data = pyg.open('./ValidationMetrics/PSL_SON.nc')    

if not (np.max(psl_DJF_metrics(metrics=0)[:] - psl_DJF_metric_validation_data.PSL_SH[:])) < 1e-10 and \
        (np.max(psl_DJF_metrics(metrics=1)[:] - psl_DJF_metric_validation_data.PSL_NH[:])) < 1e-10:
...
if not (np.max(psl_MAM_metrics(metrics=0)[:] - psl_MAM_metric_validation_data.PSL_SH[:])) < 1e-10 and \
        (np.max(psl_MAM_metrics(metrics=1)[:] - psl_MAM_metric_validation_data.PSL_NH[:])) < 1e-10:
...
if not (np.max(psl_JJA_metrics(metrics=0)[:] - psl_JJA_metric_validation_data.PSL_SH[:])) < 1e-10 and \
        (np.max(psl_JJA_metrics(metrics=1)[:] - psl_JJA_metric_validation_data.PSL_NH[:])) < 1e-10:
...
if not (np.max(psl_SON_metrics(metrics=0)[:] - psl_SON_metric_validation_data.PSL_SH[:])) < 1e-10 and \
        (np.max(psl_SON_metrics(metrics=1)[:] - psl_SON_metric_validation_data.PSL_NH[:])) < 1e-10:
...
u_data = pyg.open('./ValidationData/ua.nc')
...
if not (np.max(edj_metrics(metrics=0)[:] - edj_metric_validation_data.EDJ_SH[:])) < 1e-10 and \
        (np.max(edj_metrics(metrics=1)[:] - edj_metric_validation_data.EDJ_NH[:])) < 1e-10:
...
if not (np.max(stj_metrics(metrics=0)[:] - stj_metric_validation_data.STJ_SH[:])) < 1e-10 and \
        (np.max(stj_metrics(metrics=1)[:] - stj_metric_validation_data.STJ_NH[:])) < 1e-10:
...
olr_data = pyg.open('./ValidationData/rlnt.nc')
...
if not (np.max(olr_metrics(metrics=0)[:] - olr_metric_validation_data.OLR_SH[:])) < 1e-10 and \
        (np.max(olr_metrics(metrics=1)[:] - olr_metric_validation_data.OLR_NH[:])) < 1e-10:
...
pr_data = pyg.open('./ValidationData/pr.nc')
surf_latent_heat_flux_data = pyg.open('./ValidationData/hfls.nc')

#make the axes into the right format
pr_data = pr_data.replace_axes(time=time_axis)  
surf_latent_heat_flux_data = surf_latent_heat_flux_data.replace_axes(time=time_axis, lat=pyg.Lat(surf_latent_heat_flux_data.lat[:]))  
...
if not (np.max(pe_metrics(metrics=0)[:] - pe_metric_validation_data.PE_SH[:])) < 1e-10 and \
        (np.max(pe_metrics(metrics=1)[:] - pe_metric_validation_data.PE_NH[:])) < 1e-10:
...
uas_data = pyg.open('./ValidationData/uas.nc')
...
if not (np.max(uas_metrics(metrics=0)[:] - uas_metric_validation_data.UAS_SH[:])) < 1e-10 and \
        (np.max(uas_metrics(metrics=1)[:] - uas_metric_validation_data.UAS_NH[:])) < 1e-10:
